...
**Resolved in FE-42 ([OBT-384](https://linear.app/shema-obt/issue/OBT-384)), 16/set/2026 — Levi Gomes.** Where "prod" actually is, and the one thing the issue refused to let be a default. `.github/workflows/deploy.yml` builds on a merge to `main`, pushes to Artifact Registry in `us-central1` and deploys the Cloud Run service `project-management-ecosystem`; `docs/deploy.md` is the runbook. Three rules:

- **The service is not publicly reachable, and the workflow proves it on every run.** `--no-allow-unauthenticated`, with `roles/run.invoker` granted to named accounts — never `allUsers`, and never `allAuthenticatedUsers`, which means *any Google account on earth* and reads like an org restriction without being one. A step after the deploy reads the service's IAM policy back and fails the job if either member is there, so the property is asserted rather than remembered — and `src/__tests__/deploy.test.ts` pins the flag, the guard step, the mount path, the port and the region against the files they have to agree with, so the promise is checkable in the suite by someone with no GCP credentials at all. **The suite is not a merge gate today, and the wording here says so rather than implying otherwise** — raised in the PR #54 review, 16/sep/2026: `lint.yml` runs only ESLint and `tsc -b` on a pull request, so `npm test` runs when somebody runs it. The step inside the workflow is what holds the property on every deploy; the test file is the check you run before merging a change to `deploy.yml`. Wiring vitest into CI is one line in `lint.yml`, which FE-40 owns. It is §6.1 applied to the deploy: wave 1 shows 127 real projects, real team names and real countries, some of them `sensitiveCountry`, and fixture data derived from the Notion export is still real information about real people in real places. IAP is the next step and not this one — practical IAP needs a load balancer and a domain, and the domain is out of FE-42's scope; `docs/deploy.md` §1 carries the migration path and the table of who has access.
- **The secret is mounted, not passed as an environment variable — a conscious divergence from `meaning-map-ui`.** The reference reads `BACKEND_URL` out of Secret Manager in a workflow step and hands it to `--set-env-vars`, which puts the value in the service definition where anyone with `run.viewer` can read it, and in the CI log's shape. FE-41's entrypoint already reads `/run/secrets/.env`, so the deploy uses `--set-secrets=/run/secrets/.env=projects/shemaobt-secrets/secrets/project_management_ecosystem_env:latest`: the value never enters the runner, the workflow needs no Secret Manager read of its own, and rotation is a new secret version instead of a workflow change. The grant is **per secret** to the runtime service account, never `secretAccessor` on the project.
- **An image is addressed by commit SHA, and a rollback is a redeploy, not a rebuild.** The build pushes `:${GITHUB_SHA}` and `:latest`, and **`latest` is never deployed** — the deploy always names an immutable tag, so what is running is answerable from the service description alone. `workflow_dispatch` takes an `image_tag`: when it is filled the job skips build and push, checks the tag is in Artifact Registry and deploys it. A rebuild of an old commit is not the old image (transitive dependencies moved, the base image moved), and the minute it costs is the minute you do not have.
