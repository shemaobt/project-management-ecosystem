...
- **§4.1.1 holds on the reading side**: the page reads `projectsStore`, hydrating once, never the fixture module directly. `HomeView` is the presentational seam the render tests exercise — a static render reads a zustand store's initial state, not its mutated one, so the store-wired `HomePage` and the testable view are separate exports of the same file.
