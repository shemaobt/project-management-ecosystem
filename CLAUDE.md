...
- **The withholding is visible, not silent.** A withheld record carries `locationWithheld: true` with the region label sitting where the location would be; the CSV adds a *País sensível* column; and both formats open with a header stating what the file contains, when it was generated, that it is confidential, and how many locations were withheld (`export_withheld_count`) — the same keys the dialog shows before the download, one owner per sentence. A year with no sensitive projects gets no withheld note rather than a "0 recolhidos" line.
- **CSV hygiene is a function, not a habit.** `csvCell` in `src/utils/export.ts` quotes and escapes every field and neutralises anything a spreadsheet would read as a formula (`=`, `+`, `-`, `@` leads get `'`); the file opens with a BOM so non-Latin names (`Waima’a`, `Ngäbere`) survive Excel; the separator is `;`, the pt-BR default. Labels come from the existing vocabulary owners — `STATUS_LABEL_KEYS` was **promoted** from a private copy in the Sidebar's `sections.ts` to `src/constants/status.ts` beside `HEALTH_LABEL_KEYS`, because a third private copy is exactly what §13's one-owner rule forbids.
- **Import validates everything before applying anything.** `parseProjectsImport` accepts only the raw project list (the prototype's backup contract): the first broken record rejects the whole file with its 1-based index, a duplicated id rejects it too, and the **redacted export is recognised and refused whole** (`import_is_export`) — importing the report would replace full records with reduced ones, which is the destructive round-trip the redaction makes possible. Apply is one operation, `projectsStore.importProjects`, so all-or-nothing is structural rather than promised. The dialog states the rule (`import_desc`) and, on refusal, states that nothing entered (`import_none_applied`).
