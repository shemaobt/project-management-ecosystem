...
- **The shell owns one anchor and every screen owns the shortcut to it.** `<main>` carries `id="conteudo"` (`CONTENT_ANCHOR_ID`, `src/utils/focus.ts` — one owner, never a second literal) and `tabIndex={-1}`, and the first focusable node of the document is a skip link pointing at it. Without it, reaching the content by keyboard cost 13 header controls plus 6 nav links, on every page, every time.
