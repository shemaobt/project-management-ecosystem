...
- **Ink weights** — a palette colour that has to carry *small text* on a light surface has a darker sibling at the same hue: `azul-ink` `#406560` for azul (the prototype's own value in `.tag.azul` and `.need-status-tag.status-in-progress`), `status-attention-fg` `#8B6018` and `deadline-soon` `#A06C12` for the amber, and — since FE-47 — a heavier weight for the text a status pill carries over its own translucent wash: `status-good-ink` `#5B5F35` for the green, `status-attention-ink` `#7E5716` for the amber. They are the same series, not new colours — surfaces and *decorative* strokes keep the base token, text takes the ink. `src/styles/__tests__/tokens.test.ts` pins the hue relationship and the contrast that justifies each one; **do not add a sixth value to a family without it.**
