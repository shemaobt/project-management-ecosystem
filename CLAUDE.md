...
- **HTTP**: Axios — a single client in `src/services/api/` (the import path is `src/services/api`) with JWT auth interceptors. Written by INT-01 ([OBT-406](https://linear.app/shema-obt/issue/OBT-406)); wave 1 made no API calls and imported `axios` nowhere
...
├── hooks/
├── services/               # api/ — the single Axios client, the failure taxonomy,
│                           # the in-memory token store and the fixture/API switch (§8)
...
> **From INT-01 on, nothing imports `src/fixtures` directly except the API layer itself.** Stores and screens import the same namespaces from `src/services/api`, which serves each one from the real client or from the fixture double per the table in §8. **The fixture layer is unchanged and is not going away** — it is the test double the whole suite runs on, and `VITE_DATA_SOURCE=fixtures` is how you work on a screen with no backend. Everything below still describes it exactly.
...
Plus the project record (`modals.jsx`) as FE-20…28 / BE-06 / INT-03, Início (hero + indicators, §5.8) as FE-30, and the header's global dialogs (§5.10) as FE-39.
...
**Integrated in INT-03 ([OBT-408](https://linear.app/shema-obt/issue/OBT-408)), 11/sep/2026 — Levi Gomes.** The ten tabs read `GET /api/shema/projects/{id}` and write through `PATCH` / `POST`, against BE-06. Editing gains its first real failure mode here, and seven decisions the tab issues and INT-04/INT-05 inherit:

- **The ficha has its own store, and it persists nothing.** `projectRecordStore` owns one record, its version and the result of the last save attempt; `projectsStore` stays where the still-fixture-backed screens read it. The record belongs to the server, so a copy in this browser would be a second answer to *what does this record say* — what survives a reload is the **draft**, which `recordStore` already persists and which is the only thing losing costs anybody work. `PROJECTS_VERSION` is therefore untouched.
- **One request carrying the diff, not one per tab.** `changedFields` sends exactly the fields the coordinator changed: a field a tab did not touch never travels, so tab 3 cannot write tab 7's values as they stood when tab 3 was opened — which is the overwrite the issue names. Cutting by tab instead would either still send that tab's untouched fields (the same defect, one tab narrower) or need one request per tab, and **a twenty-row progress table has to be one write with one result**. `TAB_FIELDS` in `src/constants/recordFields.ts` is what attributes each changed field to its tab, for the report, for locating a refusal and for the pending list below.
- **A conflict re-reads the record underneath the draft, and stops there.** The draft is an overlay of the fields somebody typed, so taking the server's record under it keeps every one of them and leaves the next attempt quoting a version that exists. Retrying by itself would be last-write-wins with an extra round trip, so pressing save again is the coordinator's decision. The banner names *who* saved, *what* moved, and — the sentence that matters — **the overlap**: *somebody else changed this record* can be answered by saving again, *you and Maria both changed the status* cannot be answered without a person looking.
- **Only a save the server accepted takes anything out of the draft**, and it takes exactly what the server took (`settleDraft`). A conflict, a refusal and a dead connection all leave every keystroke where it was, which is what lets the same button be pressed again. A create carries whatever is left to the minted slug rather than leaving it under `novo`, where it would greet the *next* new record as its own.
- **BE-06's write is narrower than the ten tabs, and the tab says so before anything is typed.** The health projection has one writer and it is the assessment (§5.2, BE-07), the need items have their own endpoint (BE-08), and media and materials have no serving path for their bytes. `SERVER_WRITABLE` and `PENDING_WRITE` are the one owner of that list; the four affected tabs carry a note, and a save reports what it kept here. This is a **declared divergence from FE-44 §10**, which routes the whole record through this one `PATCH`. A field outside the allowlist is never sent — the body model forbids unknown keys, so one extra field is a 422 on the whole save.
- **Status, health and staleness come from `derived`, with no client fallback.** `src/components/pages/ficha/__tests__/serverDerivations.test.ts` fails the build if any file under the ficha names one of the nine derivations. The three utils are **not deleted**: eight fixture-backed screens and `fixtures/projectBrowse.ts` — the browse endpoint's own test double, which has to reproduce the server to stand in for it — still read them. What INT-03 removes is the ficha's second answer, because a fallback agrees with the server right up to the moment it does not, which is a slow network.
- **`phases[].date` is free text on the record, and the ficha stopped reading it as a day.** *"2º semestre"* is a real answer to *when* and the export carries answers like it; `formatDate` rendered them as `Invalid Date`, and the `type="date"` input showed an empty box where there was data. `isCalendarDay` in `tabs/objetivo/phases.ts` is the single owner of the distinction. Found by integration, not by review.

> **Verified against BE-06's own OpenAPI schema, not against a running server.** The API is not on staging yet and its migrations need Postgres, so the wire shape was checked by generating the schema from the branch's code and diffing it field by field: 74 record properties, every one's nullability, every sub-shape's keys, the update model's accepted fields and the two routes. End-to-end against a live server waits for the merge into `dev`.
...
- **Avaliação de Saúde** — filled **in-app** by the OBT Lab mentor during an online meeting. A 4-dimension wizard (`health-modal.jsx`) with guiding questions:
  ...
  **Built in FE-37 ([OBT-379](https://linear.app/shema-obt/issue/OBT-379)), 16/aug/2026 — Daniel Oliveira**, at `/formularios/avaliacao/:projectId`, which is where FE-35's `forms_open_health` button now leads. Six decisions:

  - **The guiding questions are real, and the issue's ⚠️ resolves in the affirmative.** FE-37 was told to check whether they were placeholders and, if so, to ship them visibly marked as pending rather than invent plausible-sounding ones. `health-modal.jsx` carries **sixteen** — four per dimension — and they match what `Formulários - Apresentação Detalhada.html` slide 06 quotes. Nothing was invented and nothing needs the client. They live in `src/constants/healthQuestions.ts`, whose `QUESTIONS_PENDING_CLIENT` is an empty array asserted by a test: it is the lever to pull if a dimension ever *does* need client input, so the pending state has a home before it is needed.
  - **The per-dimension note is the data; the running note is a reading of it.** The prototype joins the four notes into one `healthNotes` blob (`"Emocional: …\n\nRelacional: …"`), which is lossy and unqueryable — and the issue is explicit that the note matters more than the rating. `HealthAssessment` gains `dimensionNotes`, and `compileNotes` derives the blob at write time so FE-25's existing display keeps working unchanged. One source, one derivation, both pinned.
  - **Skipping is an answer.** A dimension left alone stays `""`, `overallOf` returns `na` rather than `boa`, and the step says so in words. This is §5.2's rule reaching the wizard: a project nobody talked to is *not heard*, never *well*.
  - **Autosave is per step, per project, in `assessmentStore`.** The DoD asks for a test that interrupts mid-wizard and asserts nothing is lost; `assessmentAutosave.test.ts` writes two dimensions, reads the persisted payload back into a cleared store and resumes from it. It also asserts one project's draft never leaks into another's.
  - **The suggestion never becomes the decision.** `pastoralSuggestion` returns `suggested` **plus the reasons** — which dimension came back how — and the screen prints them. `pastoral` stays `"nao"` until a person picks otherwise, asserted by a test. Auto-applying would turn a pastoral conversation into an automated judgement, which is the thing this product's culture rejects.
  - **Saving goes through `recordAssessment`, and the wizard owns only what is its own.** `applyAssessment` in `src/utils/assessment.ts` is the one place the finished draft becomes a project: it delegates the (history, flat fields) pair to `recordAssessment` per §5.2 and adds only the pastoral answer and — **when one was actually written** — the prayer request and its consent. An earlier revision of this wizard appended to the history and projected the flat fields by hand, which broke two things the §5.2 rule exists to protect: a backdated conversation left the flat fields showing the *older* reading, and a project whose record predates `healthHistory` lost it. It also wrote `prayerRequests: ""` on every save, deleting an existing request as a side effect of an unrelated action — which §6.2 forbids even when consent is withdrawn. Raised in review; all three are pinned by tests that fail against the old code.
  - **No file, anywhere in the flow — and now it is enforced, not promised.** FE-35's `forms_health_mechanism` tells the user the assessment produces no file; `wizard.test.ts` walks every `.tsx` under the flow and fails on `createObjectURL`, `new Blob(`, a `download` attribute or an import of `utils/export`. That guard matters more since FE-39 shipped `src/utils/export.ts`: the export machinery now exists one import away. Proven to catch the defect before it was accepted.

  **Single choice goes through `ui/RadioGroup`, including where it does not look like a radio.** The pastoral answer is three mutually exclusive pills, and the first cut hand-rolled them as plain buttons carrying `aria-pressed` — three tab stops, no arrow keys, the same defect FE-46 fixed in `RatingChoice`. `primitives.test.ts` does not catch that shape, because it scans for `role=` and `aria-checked=` and this one announced itself differently; widening it to ban `aria-pressed` outright would flag five legitimate independent toggles (the filter chips, the metaphor pill, `Chip`). The rule is the one in §3.1 — mutually exclusive means `RadioGroup` — and here it is asserted directly instead: the three controls all carry `tabindex="-1"`, which is what tells them apart from three separate tab stops, and the markup carries no `aria-pressed` at all. Raised in review.

  The wizard writes the prayer request through **FE-25's `PrayerConsent`, the same component** — a test asserts the rendered consent block is byte-identical to rendering that component directly, so a second consent path cannot appear without failing. A new assessment starts at `DEFAULT_PRAYER_VISIBILITY` rather than inheriting the project's last choice: consent is given per request, not once and forever. `avaliacao/index.tsx`, `types/assessment.ts` and `utils/assessment.ts` are therefore added to FE-32's raw-prayer-field allowlist, deliberately — the wizard is a writing surface for those fields, exactly like `SaudeForm`.

  **Integrated in INT-04 ([OBT-409](https://linear.app/shema-obt/issue/OBT-409)) · BE-07 ([OBT-396](https://linear.app/shema-obt/issue/OBT-396)).** The issue's Scope names `src/components/pages/ProjectRecord/HealthAssessment/**`, stale English wording per §10 — the real files are `avaliacao/**` (the wizard) and `ficha/tabs/saude/**` (the record's own history display). Five decisions:

  - **`applyAssessment` stopped being the save.** It wrote straight to `projectsStore` against the wave-1 fixture, with no server in the loop at all — wave 2's whole point. `AvaliacaoPage` now opens the record through `useProjectRecordStore` (INT-03's own seam: one record, its version, what the last write said) rather than `useProjectsStore`, and a new `submitAssessment` action there calls `POST /shema/projects/{id}/health-assessments` (`src/services/api/healthAssessments.ts`) and replaces `record` with what the server answers. `applyAssessment`/`toAssessment`/`recordAssessment` did not become dead code — they are exactly what the fixture double (`fixtures/projectRecord.ts`'s `submitAssessment`) reproduces server behaviour with, the same append-and-reproject rule BE-07 documents, now also stamping the three keys BE-07 adds (`author`, `questionSetVersion`, `overall`) via a new optional `meta` parameter on `toAssessment`/`applyAssessment`.
  - **The overall reading is a preview until the server answers.** `Completion.tsx` still runs `overallOf(draft)` locally — the mentor needs *some* reading to weigh the pastoral suggestion against before ever pressing Save — but the badge now carries `hw_preview_tag` and `hw_overall_preview_note` says outright that the server's own answer is what counts. Nothing downstream (the saved record, the toast, the history) ever reads this local value; `submitAssessment`'s response is the only source once a reading is actually filed.
  - **Critical says so before the button, not after.** When the preview lands on `critica`, the summary shows `hw_critical_notice` — the region's coordination is notified the moment this saves — in the same card as the preview badge, on the review step, ahead of Save. The issue's own wording ("someone should know their assessment will reach others before they submit it, not after") is why this is not folded into the generic `hw_notify` footnote that was already there.
  - **A failed submission never touches the draft.** `AvaliacaoPage.onFinish` calls `discardDraft` and navigates away only inside the `"saved"` branch; `"invalid"` and `"failed"` set a local `submitOutcome` (rendered by the new `SubmitOutcomeNote`, the wizard's own narrower cousin of the ficha's `SaveOutcomeNote`) and leave the step, the ratings and the notes exactly where they were — the Save button is the retry, disabled only while a request is in flight. `assessmentStore`'s own autosave (FE-37) was already the "do not lose a partly-finished assessment" half of this; INT-04 is what keeps a network failure from discarding it a second later.
  - **History reads what the record already carries, and the version is named rather than assumed.** `read_record.py` joins `healthHistory` into the record response, so `AssessmentHistory` (in the ficha's Saúde tab, per §5.2) needed no second fetch — `HealthAssessment` grew `author`, `questionSetVersion` and `overall` as optional fields, and the component now shows the filer beside the assessor, an `overall` badge (`entry.overall`, falling back to `overallOfEntry` only for an entry a server never stamped), and a version tag. Only one question set is published today (BE-07's version 1, `health_q_*`, identical to `HEALTH_DIMENSIONS`), so there is no older wording to render yet; the tag is what stops a future, unknown version from silently reading as today's four questions, and a `null` version (a row carried out of the flat fields before this endpoint existed) says plainly that it answered no questionnaire.

  **One field the wizard collects does not travel: `hw_overall_note`.** BE-07's submission derives `notes` from `dimensionNotes` alone (FE-37's "one source" rule, §5.2) and has no column for a note that cuts across all four dimensions; stuffing it into one dimension's note would misattribute it. The field stays in the wizard — still autosaved — with `hw_overallnote_local_only` stating plainly that it is not sent, the same what-holds-today idiom `f_sensitive_on_pending` uses.
...
**Integrated in INT-10 ([OBT-415](https://linear.app/shema-obt/issue/OBT-415)), 11/sep/2026.** `regionsStore.saveTeams` now writes through `PUT /api/shema/regions/{key}/team`, one call per dirty region, and adopts the server's `SaveOutcome` and `RoleChange[]` rather than computing them client-side — `changedBy`/`changedAt` are the authenticated actor and the server's own clock (`save_region_team.py`), never the client's guess. `diffTeams` still runs first, purely to decide which regions changed and are worth a call; the diff it produces is discarded once the server answers. A partial failure (one region's write refused, another's accepted) applies what succeeded and toasts the rest, rather than failing the whole save. `hydrate()` also reads `GET /regions/role-changes` for the audit trail, best-effort — a role without `coordinator` gets an empty trail rather than a failed screen, since the seats themselves render without it. The fixture side of `regionsAPI.saveTeam` stays a pure function of `(from, to, changedBy, now)`, reusing `diffTeams` for one region, so the wave-1 invariant holds unchanged: the fixture module never mutates, and `regionsStore` is still the only place a saved name lives before it reaches the server.
...
- **A notification is an output path, enforced by reference.** Prayer text enters only through `buildPrayerRequests` — the §6.2 gate owner; FE-32's scan test already fails any file that reads the raw fields. Location goes through `getLocationDisplay` (FE-14's owner): a withheld project's entries carry `country: ""` and no location content, and the row title is language · base — the base staying verbatim, per FE-15's open gate (§5.1). Tests pin the absences over the built list, not the card.
- **The cap is per recipient, after routing.** `routedNotifications` composes build → route; `visibleNotifications` applies the prefs over that routed list and only then keeps 30 (`NOTIFICATION_LOG_LIMIT`). The pipeline is two functions so the bell memoizes the halves separately — typing a delivery address changes prefs, and prefs must not re-run the derivation over every project. Capping before routing would let one region's volume evict another recipient's entries. The badge counts unread over exactly the list the panel shows — a count never promises rows the panel does not return.
...
- **Prefs are honest about what works today.** The channel toggles record the choice and `notif_channels_hint` states that delivery is wave 2 (FE-21's what-holds-today split); `enabled` and `when: "urgent"` really filter the in-app list; "Só projetos que mentoro" matches the session name against `mentor` and a null persona name matches nothing; and **"Lista personalizada" ships a real picker** — an option that filters nothing would be a lying label, the FE-30 count rule applied to a preference. The address fields ship empty with the channel label as placeholder: the prototype's `NOTIF_DEFAULTS` seeds a real person's e-mail and a masked mock phone, and FE-36's no-hardcoded-names precedent applies to a delivery address exactly as it does to an org chart. Raised in the PR #38 review, 15/aug/2026.
...
### 5.10 Modais globais do cabeçalho — Receber, Exportar, Importar, Recarregar, Link do líder

Built in FE-39 ([OBT-381](https://linear.app/shema-obt/issue/OBT-381)), 15/aug/2026 — Levi Gomes. The five header actions FE-06 shipped as pending toasts now open dialogs (`layout/AppHeader.tsx` + `pages/dados/`; the issue's Scope spelled `DataDialogs/`, §5.8's precedent renamed it). `toast_pending` retired with its last consumer. Seven decisions:

- **The export is a report that leaves, not a backup that returns.** The prototype's `handleExport` dumps the raw records; the issue's own rules make that impossible, so the export became an **allowlist**: `redactProjectForExport` in `src/utils/privacy.ts` is the single redaction owner, and a field added to `Project` later stays *out* of the file until someone puts it in — the safe default for the product's highest-risk artifact. Notes are asked to `canExportNotes` under `EXPORT_AUDIENCE = "publico"` (the file gets emailed onward); prayer text enters only through `buildPrayerRequests` — FE-32's owner, so the raw fields are never read and the scan test needed no new entry; location goes through `getLocationDisplay`; personal contacts, people names and media never enter the allowlist at all. `export.test.ts` pins the DoD's guarantee over both serialized formats, not over the record.
...
> ⚠️ **Open for the privacy discussion, not decided here.** Wave 1 records **when** someone entered the network (`addedAt`) so the question is answerable, and states the obligation in the UI (`int_privacy_note`). It does **not** record *the basis on which the contact is held*, and there is no review or expiry. Three things need an owner before this data leaves the browser: what consent was given and how it is evidenced, how a person outside the platform asks to be removed when they cannot log in, and what happens to a contact nobody has used in a year. **BE-13** stores this network server-side (corrected from BE-09: `shema-api`'s `docs/shema.md` §1.3 C3 moved the aggregate, and INT-10 is the screen the graph names as blocked by BE-13, not BE-09); none of the three can be answered by the frontend alone, and shipping the storage before answering them is how silent retention starts.

**Integrated in INT-10 ([OBT-415](https://linear.app/shema-obt/issue/OBT-415)), 11/sep/2026 — and the server answers more of §10 item 8 than this section expected.** `IntercessorEntry` on the wire has **no `contact` field at all** — the collection carries `contactChannel` and a masked `contactHint` only, and the real string is one person, one call, logged: `GET /prayer/intercessors/{id}/contact`. `Intercessor` split accordingly: `src/types/prayer.ts` now types the list shape as `IntercessorEntry` (no `contact`), and a contact reaches the screen only through an explicit action — the row's ligar/mensagem buttons and the edit dialog's own reveal step, never a bulk read. This is stricter than what FE-33 shipped (`cards.jsx`-style, contact printed in every row) and behaviour is where a later, better-evidenced design wins over an earlier one built without the server to react to.

**Consent is per context, not a single flag, and the create form now asks for it.** The server's ladder — `network` (may we hold and reach you), `directory` (may we list you internally), `partner-export` (may you leave in something that goes to a partner) — answers §10 item 8's first question: `consentBasis` is required to create a record at all (the floor, `network`), and a second, explicit "listar no diretório interno" checkbox grants `directory` before the person appears in the list. A person held but not listed counts into `withheldCount` — shown in words above the list, never silently absent — the same "a silently incomplete list is its own hazard" reasoning §8.1 rule 2 already applies to the Atlas. `partner-export` has no UI yet; nothing in wave 1 or this integration exports the network, so the question has no caller to answer it for.

**`sensitiveCountry` rides on the network row too**, the same flag `CLAUDE.md` §6.1 already carries from a project's `location` to its personal contacts — visible on this read (a coordination surface), and the server's job to withhold on every path that leaves, none of which exist yet on this screen.
...
- **Ink weights** — a palette colour that has to carry *small text* on a light surface has a darker sibling at the same hue: `azul-ink` `#406560` for azul (the prototype's own value in `.tag.azul` and `.need-status-tag.status-in-progress`), `verde-claro-ink` `#6B713E` for the green (the wizard's selected rating, the `equipe` and `mídia` tab markers, a fulfilled need, a reported project), `status-attention-fg` `#8B6018` and `deadline-soon` `#A06C12` for the amber, and — since FE-47 — a heavier weight still, for the text a status pill carries over its own translucent wash: `status-good-ink` `#5B5F35` for the green, `status-attention-ink` `#7E5716` for the amber. **The green therefore carries two inks and they are not interchangeable:** `verde-claro-ink` is the weight that reads on an opaque surface, `status-good-ink` the one it takes over the 14% wash on `bg-muted`, where `verde-claro-ink` is only 3.70. They are the same series, not new colours — a *decorative* surface or stroke keeps the base token, and the ink goes wherever legibility is at stake: the text, **and the fill under it when the base fill is what makes the text fail** — FE-47's health dot is `bg-verde-claro-ink`, not `bg-status-good`, because branco on the base green reads 3.99. `src/styles/__tests__/tokens.test.ts` pins the hue relationship and the contrast that justifies each one; **do not add another value to a family without it.**
  ...
  - **A wash is a different surface everywhere it sits, and a pill ported verbatim can fail on all of them.** FE-47 ([OBT-424](https://linear.app/shema-obt/issue/OBT-424)), 14/sep/2026 — Levi Gomes: the health and staleness pills share three tones and none passed AA on every surface they render on. `criticalTone` (telha on `accent-soft`, 3.69) and the `na` pill (`fg-subtle` on `bg-muted`, 2.95) are the two pairs FE-31 had already replaced in the Rhythm, and take the same siblings — `accent-press` (6.31) and `fg-muted` (5.87). `goodTone` and `attentionTone` sit on a 14% wash of their own colour, so the number depends on what is behind the pill: `verde-claro` reads 3.71 on `elevated` and 3.13 on `muted`; `status-attention-fg` 4.86 and 4.11. No declared green or amber clears 4.5 over the wash on `bg-muted` (`verde-claro-ink` 3.70), thinning the wash does not get there either (4.02 at 6%), and the pill does live on `bg-muted` — the Saúde tab's prayer block — so neither of the issue's cheaper exits was available. Each family gets one heavier ink, same hue as the base, lightness cut until the worst surface clears 4.5 with a margin: `status-good-ink` (4.79 on `muted`, 5.68 on `elevated`) and `status-attention-ink` (4.77 / 5.64). The washes are untouched. The dots — the `✓ ! × –` glyph the Atlas, the Diário, the Saúde tab and the history actually render — were worse: white on `status-good` is 3.99 and on `status-attention` **2.66**, the lowest number in the design system. Their fill moves to the sibling already declared and already used by the wizard's selected-rating buttons, `verde-claro-ink` (4.73) and `status-attention-fg` (5.07). The `na` dot drops its `opacity-60`: a veil over the whole element dims the glyph together with the fill, so a pair that measures 6.03 rendered at about 2.5. `tokens.test.ts` measures every health and staleness pill composited over `elevated`, `canvas`, `muted` and `paper` — a wash is measured as what it paints, not as its declared rgba — asserts the sets are complete, pins both new inks to their base hue and to the reason they exist, and forbids an opacity utility in a tone that carries a glyph. `Badge`'s `accent` tone (telha on `accent-soft`, 3.69) is the same defect outside this issue's scope and stays open.
...
**Resolved in FE-43 ([OBT-385](https://linear.app/shema-obt/issue/OBT-385)), 16/sep/2026 — Levi Gomes.** The ink-weight rule above is about a *palette* colour asked to carry small text. `--fg-subtle` is the case it did not cover, because it is not a palette colour: it sits in the `--fg-*` family, which means its one job **is** small text — and at the design authority's `#8A8970` it failed that job on every light surface the interface paints, in 104 places across 64 files. Measured: **3.25** on canvas, **3.56** on `elevated`, **2.95** on `bg-muted`, **3.40** on `paper`.

- **So the fix is the token, not the call sites.** `--fg-subtle` is now `#6A6956` — hue 57.0 against the authority's 57.7, inside the tolerance `tokens.test.ts` already used for the ink families, and darkened only until the worst surface clears with a margin (**4.62** on `muted`, **5.57** on `elevated`). It stays lighter than `fg-muted`, so the three ink weights are still three. This is the first time a value declared in `design-system/colors_and_type.css` is **replaced** rather than given a darker sibling; the sibling pattern exists to leave a decorative base alone, and a foreground token has nothing decorative to leave alone. **The designer is owed this one** — the number, and the new value.
- **A token that is not ink must not ride on one.** `--night-paused` was `var(--fg-subtle)`: a marker on the night globe, where darkening makes it *less* visible. It now carries the literal `#8A8970`, and the test measures each one against the surface it actually sits on.
- **`Badge`'s `accent` tone is closed.** §7.1 recorded telha on `accent-soft` (3.69) as the same defect outside FE-47's scope; FE-43 moves it, and the seven other text sites that carried the same pair, to `accent-press` (6.31). What keeps `telha` over `accent-soft` is the `:hover` of icon-only controls, where 3:1 is the floor — seven files, named in `GLYPH_ONLY` in `tokens.test.ts`, with the exemption asserted as complete so an eighth cannot join quietly.
- **A translucent veil over ink is a contrast change nobody writes down.** `text-on-brand/80` on the chip count read 3.62 over telha; the `/80` is gone. Same shape as FE-47's `opacity-60` on the `na` dot.
- **White is not ink on every brand fill.** `fg-on-brand` over `verde-claro` is 3.99 and over `azul` is 2.30 — below even the 3:1 of a graphical object. `Button`'s `green`, `IntakeSuccess`'s mark and the Atlas stamp's `completed`/`planned` take the `-ink` siblings, the same move FE-47 made for the health dot.
- **The proof is in the repo, and it is two kinds.** `scripts/contraste.mjs` reads `index.css`, prints the ink × surface matrix and scans the source for the fill/ink pairs actually written, reporting what falls under 4.5. And, because a wash composited over a real background is a number no scanner can reach, the same pass measured the **rendered DOM** in a headless Chrome across 34 surfaces — nine routes, the ten record tabs in both modes, the five header dialogs — and found four failures the static scan could not see. All four are fixed; the sweep now reports zero.
...
- Scale (px): display 88 · h1 56 · h2 40 · h3 28 · h4 22 · lead 20 · body 16 · small 14 · micro 12 · eyebrow 13
  - **The four display sizes are fluid, and the number above is the ceiling.** Since FE-43 ([OBT-385](https://linear.app/shema-obt/issue/OBT-385)), 16/sep/2026 — Levi Gomes: `--fs-display`, `--fs-h1`, `--fs-h2` and `--fs-h3` are `clamp(floor, vw, ceiling)`, with the ceiling equal to the value listed here, so nothing moves at 1280px. The floor exists because a fixed 56px `<h1>` does not fit a 320px screen — measured at **427px** of `scrollWidth` in a 320px viewport, which is WCAG 1.4.10 broken by the type scale itself, on every screen at 200% zoom. Floors: display 44 · h1 32 · h2 26 · h3 22. Same shape as `--container-pad`, which was already a clamp. **Pending the designer's ratification of the floors.**
  - **A page's width is the prototype's, and the container scale does not get to round it off.** Raised in the PR #55 review, 16/sep/2026: FE-43 added a guard forbidding a page wrapper from pinning `max-w-` in pixels, and the one wrapper still doing it — the prayer wall — was snapped to `--container-max`, which is **1200px** where `app.css` `.or-wrap` says **1080px**. A guard that can only be satisfied by changing a value is a guard that launders a layout change through an accessibility pass, so the width got a name of its own instead: `--container-mural: 1080px`. The scale now reads 1500 (`wide`) · 1200 (`max`) · 1080 (`mural`) · 920 (`reading`) · 760 (`narrow`).
  - **Início is the one page whose content block does not use `--container-pad`, and the exemption is named.** Same review: the globe block sits directly under `Hero`, and the prototype pins the hero's padding in pixels (`.hero { padding: 36px 32px 28px }`, `20px` on the phone breakpoint) rather than using the design system's fluid token. Moving only the globe to `--container-pad` pushed the two edges 24px apart from ~900px up. The block follows the hero, `HERO_ALIGNED` in `acessibilidade.test.ts` carries the exemption with its reason, and a second test asserts the two paddings still match — so the alignment is pinned rather than remembered.
...
**Extended in FE-43 ([OBT-385](https://linear.app/shema-obt/issue/OBT-385)), 16/sep/2026 — Levi Gomes.** A visible ring answers *where* the focus is; it says nothing about the focus having nowhere to be.

- **The shell owns one anchor and every screen owns the shortcut to it.** `<main>` carries `id="conteudo"` (`CONTENT_ANCHOR_ID`, `src/utils/focus.ts` — one owner, never a second literal) and `tabIndex={-1}`, and the first focusable node of the document is a skip link pointing at it. Without it, reaching the content by keyboard cost **14 stops** — 8 header controls and 6 nav links, counted in the browser, not estimated — on every page, every time.
- **Radix returns focus to the trigger if the trigger still exists, and three wave-1 flows close a modal by unmounting it**: the Ficha *is* the route, so closing navigates away and destroys the card that opened it; logging a Rhythm meeting swaps "Registrar" for "Desfazer" while the dialog is still open; confirming an intercessor removal deletes the row the button lived in. In all three the focus landed on `<body>`, which is where a screen reader loses its place. `DialogContent` and `SheetContent` therefore run `scheduleFocusRescue()` **after** the caller's own `onCloseAutoFocus`, and it only acts when focus actually ended up nowhere — when the trigger survives, Radix is left alone.
- **A scroll container whose children are not focusable is unreachable by keyboard** (WCAG 2.1.1). `ui/Table` wraps its table in `overflow-x-auto`, so the wrapper is now a named, focusable region and the name is a **required** prop.
...
- Dev: Vite proxy `/api` → `VITE_API_PROXY_TARGET` (default `http://localhost:8000`); the client's own prefix is `VITE_API_BASE_URL` (default `/api`)
- Prod: `BACKEND_URL` injected at container entrypoint — and **required**: `docker-entrypoint.sh` refuses to start when it is unset or has no `http(s)://` scheme, so a deploy always delivers it, even in wave 1 with a placeholder (FE-41, [OBT-383](https://linear.app/shema-obt/issue/OBT-383), 14/set/2026 — Levi Gomes). Vite inlines every `VITE_*` value at build time, so nothing under that prefix is a secret and no secret is baked into the image. The proxy block is `location ^~ /api` — without the `^~` the static-cache regex outranks the prefix and every `/api/*.png|jpg|svg` falls through to `root` — and it declares **no CORS**: the same nginx serves the SPA and forwards the API, so the calls are same-origin, and reflecting `$http_origin` back with `Allow-Credentials` would both widen the surface and collide with `shema-api`'s own headers into two `Access-Control-Allow-Origin`. Preflight is the backend's to answer.
- Keep TypeScript types aligned with the Pydantic schemas in `shema-api`'s `app/models/`. `shema-api` produces its own OpenAPI from FastAPI — read the schema rather than inferring shapes from a sample response.
- Before adding an endpoint, check whether `shema-api` already exposes one (§3.2 reuse table).

**Built in INT-01 ([OBT-406](https://linear.app/shema-obt/issue/OBT-406)), 11/sep/2026 — Levi Gomes.** The client is `src/services/api/` — a folder whose `index.ts` answers to the import path `src/services/api` that this section and FE-44's contract always named. Six decisions every later INT inherits:

- **The switch is per namespace, in one table.** `INTEGRATED` in `src/services/api/source.ts` says, for each of the eight namespaces, whether it is served by the real client or by the fixture double, and `INTEGRATED_BY` names the issue that owns the flip. That is CLAUDE.md §10's *"one screen at a time, reversible per screen, fixtures kept behind a flag"* written as code: **INT-02…INT-11 each flip one row**, and flipping a row is where that screen's loading and failure rendering becomes its author's problem — the store already records both. `VITE_DATA_SOURCE=fixtures` forces every namespace to the double (wave-1 local dev with no backend, and what the test suite runs on); `=api` forces every namespace to the API. A value that is neither is ignored, and a test pins that a typo flips nothing.
- **The interface is enforced by the compiler, not by good intentions.** `index.ts` picks with `pick<typeof fixture.xAPI>(…)`, so a method the real client lacks fails `tsc`, and a test asserts the two sides carry the same method names, the same arity and `async` on both. **`geoAPI` is deliberately not in the table**: the contract (§9.1, §6.3) says the continent outlines are a bundled asset and not an endpoint, so the index always serves the bundle.
- **The wire is the frozen type, verbatim.** No case conversion anywhere: `src/types/` is what `GET /api/shema/*` must answer, and `GET /api/shema/session` already does (BE-03 aliases to camelCase). **The platform's own auth routes are the one exception** — `/api/auth/*` is pre-existing snake_case (`access_token`, `refresh_token`, `display_name`) and gets one named mapping of three fields in `endpoints.ts`. **BE-05 and BE-06 must serve `Project` in camelCase**; a second spelling would put a translation table on every record read.
- **A failure has ten readings and one owner.** `toApiFailure` in `errors.ts` classifies into `offline` · `timeout` · `unauthorized` · `forbidden` · `notFound` · `conflict` · `invalid` · `server` · `unexpected` · `canceled`, and `failureMessage` picks the sentence: **the server's `detail` wins only where the server knows more than we do** (`invalid`, `conflict`, `notFound`) and the console's own sentence wins everywhere else, because "Internal Server Error" is not something a coordinator on a bad connection can act on. `navigator.onLine === false` outranks a timeout code — the useful advice differs. Every consumer reads `failureMessage`; nobody writes a second sentence, except the sign-in surface, whose reading of `401` is *wrong password* and not *session expired* (`signInMessage`, one file).
  - **Reading a failure as the network takes positive evidence, not the absence of a response.** A no-response failure is only classified as `offline` / `timeout` / `canceled` when `isAxiosError` says a request was actually attempted; anything else is `unexpected`. Raised in the PR #46 review, 12/sep/2026: a plain `Error` satisfied the old shape check with `code` and `response` both undefined, so a `TypeError` thrown inside a store's `load()` — a defect of ours, nothing to do with the network — announced itself to the field as `net_offline` — *"Sem conexão com o servidor… tente de novo quando a rede voltar"* — when `net_unexpected` (*"Algo deu errado no painel antes de a chamada sair"*) is the sentence that exists for exactly this. The test doubles now carry `isAxiosError` because every error axios rejects with does — `AxiosError`'s constructor sets it, and `AxiosError.from` and `CanceledError` both go through that constructor, so a cancellation still reads as `canceled`. A double more permissive than the real thing is how the defect stayed invisible; three cases of `errors.test.ts` fail against the old reading.
- **A background read never throws at a screen.** `hydrateOnce` in `src/stores/hydration.ts` owns the rule for all five hydrating stores: a second caller awaits the first's promise instead of firing a second request, a failure is classified, announced once and recorded as `error`, and **`hydrated` stays false** so a screen cannot render an empty list as the truth. `reload()` is the same call with `force`.
- **A vocabulary the console does not know fails closed.** `readSession` refuses a role outside `SESSION_ROLES` or a region outside `REGIONS` rather than silently narrowing the scope — a narrowed scope would lie about reach, and the message says the console is behind the API.

### Auth

- **Wave 1 kept a mocked session**, and it is still the fixtures-mode implementation: `MockAuthProvider` is that code unchanged, with the dev `RoleSwitcher` it was built for.
...
- The frontend never *enforces* authorization — it only reflects it. Every rule is enforced in `shema-api`'s service layer; hiding a control in the UI is presentation, not security.

**Built in INT-01 ([OBT-406](https://linear.app/shema-obt/issue/OBT-406)), 11/sep/2026 — Levi Gomes.** The real session is `ApiAuthProvider`, entered through `POST /api/auth/login` and read from `GET /api/shema/session` (BE-03). `AuthProvider` picks between the two by `resolveSource("session")`; both fill one context, so no consumer knows which it has. Four rules:

- **No token is written to any web storage — not access, not refresh.** `src/services/api/tokens.ts` is the single owner and holds both in module memory. This supersedes an earlier revision of this section that said *"JWT access + refresh in localStorage"*: a token in `localStorage` is a token any script on the page can read, and the issue's own rule is that it must not be. **The cost is stated rather than hidden: reloading the page asks for the password again**, and the sign-in panel says so (`entrar_session_note`). ⚠️ **Closing that costs a server change, not a frontend one**: `POST /api/auth/refresh` takes the refresh token *in the body*, so a browser-only SPA cannot make it httpOnly by itself. A session that survives a reload needs `shema-api` to set the refresh token as an httpOnly, `Secure`, `SameSite` cookie on login and read it from there on refresh — at which point the restore-on-mount path is added to `ApiAuthProvider` and nothing else here changes. A test walks every file of the API layer and fails on any mention of `localStorage`, `sessionStorage`, `indexedDB` or `document.cookie`.
- **A 401 refreshes once and retries once.** Concurrent 401s share one refresh promise; the refresh itself goes out on a bare Axios call so it cannot recurse through the 401 handler; and `/auth/login`, `/auth/refresh` and `/auth/logout` never enter it, so a refused password is a refused password.
- **Expiry does not navigate.** On refresh failure the tokens are dropped and the session goes to `expired`, and `SessionGate` keeps the app mounted and opens the re-authentication dialog **over** it: the record's React state and the health wizard's step survive, and the `localStorage` drafts are never touched. An earlier revision of this section said *"redirects to `/login`"* — a redirect unmounts exactly the unsaved work the issue forbids losing. The dialog has two named exits, sign in again or sign out.
  - **A refused re-authentication is still a re-authentication.** Raised in the PR #46 review, 12/sep/2026: the failed-sign-in path set `anonymous` unconditionally, so one typo in the password while the dialog was open resolved the surface to `signIn` and the gate swapped the whole app for `EntrarView` — `keepsWorkMounted("signIn")` is `false`, so the wrong password did exactly what expiring must not do. `statusAfterRefusedSignIn` is the rule and it has one owner: a refused attempt returns the person to the surface they came from, so the dialog stays over the mounted work with the wrong-password sentence inside it. **And the session goes back with the status** — raised in the second pass of the same review, 13/sep/2026: keeping `expired` while still clearing `signed` left the app mounted under a persona that was nobody, so `personaOf` fell to `NOBODY` and `visibleRegions` to `[]`, and the surfaces behind the dialog emptied out. The one that bites is `SavedViews`, whose store is keyed by `user.id` and persisted: at `id: ""` the person's views vanish from the panel and saving one files it under the empty string, orphaned for good once the real session returns. The reachability the reviewer named — Equipe's role drafts — does not in fact lose the typed text (`edits` lives in `EquipeView`, which stays mounted, and `RegionRoles` inputs are controlled), but the cards do unmount into an empty state. The rule is now one invariant instead of five case bodies: **only `anonymous` carries no session**, so `expired` keeps the one it is asking you to re-prove and reaching `expired` by a token event and by a refusal can no longer disagree. `personaOf` moved next to the state it derives from (`apiSession.ts`) so the test can assert the persona behind the dialog is still the coordinator and not a global strategist with no regions; `apiSession.test.ts` pins the invariant over ten reachable sequences rather than per case.
  - **The status machine is a pure reducer, because a provider cannot be rendered in this suite.** `apiSessionReducer` (`src/contexts/apiSession.ts`) owns `status`, `signed` and `failure`; `ApiAuthProvider` only dispatches. That is §5.8's presentational-seam rule applied to state rather than markup — there is no DOM renderer here, so a state machine proven only by rendering the provider is not proven at all — and it settles the ordering by construction rather than by React's batching: a token event the attempt itself caused (the `signOut` the failed path performs) is ignored while an attempt is in flight, so no intermediate `anonymous` render can unmount the work between the drop and the recovery. `src/contexts/__tests__/apiSession.test.ts` replays the sequences the provider dispatches, and five of its twelve cases fail against the old rule.
- **The session's name comes from the server.** BE-03 resolves it from the org chart and falls back to `users.display_name` (`docs/shema.md` §6.3), so in API mode the fourth consumer FE-36 named — `AuthContext` — is server-side and `GLOBAL_STRATEGIST_NAME` is never read. It stays in `MockAuthProvider`, where the mocked persona still needs a name.

**Checking against a running `shema-api`.** `src/services/api/__tests__/live.test.ts` skips unless `SHEMA_LIVE_API` names a base URL, so it costs the suite nothing and is there for every later INT: `SHEMA_LIVE_API=http://localhost:8101 npx vitest run src/services/api/__tests__/live.test.ts`. ⚠️ The API does not boot against an empty SQLite file — its startup reads `bible_books` and none of its migrations run under SQLite (`docs/shema.md` §7.2) — so a local instance needs either the Postgres recipe in that repository's `RUNNING-LOCALLY.md` or, for a read-only probe, `Base.metadata.create_all` over a throwaway SQLite file plus `scripts/seed_apps_roles.py` and `scripts/grant_app_role.py`.
...
- [ ] Screens and stores read the namespaces of **`src/services/api`**, never a hand-rolled local copy and never `src/fixtures` directly — the API layer is the only importer of the double (§4.1, §8).
